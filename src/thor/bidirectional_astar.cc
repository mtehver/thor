...
void BidirectionalAStar::ExpandForward(GraphReader& graphreader,
  ...
  for (uint32_t i = 0; i < nodeinfo->edge_count(); i++, directededge++, edgeid++) {
    ...
    if (directededge->trans_up() || directededge->trans_down()) {
      ...
      edgelabels_forward_.emplace_back(pred_idx, edgeid, pred.opp_edgeid(),
      ...
      if (endtile != nullptr) {
        ExpandForward(graphreader, endtile, node, endtile->node(node),
                    edgelabels_forward_.back(), idx, true);
      }
      continue;
    }
    ...
    Cost newcost = pred.cost() + tc +
                  costing_->EdgeCost(directededge, nodeinfo->density());

    // Check if edge is temporarily labeled and this path has less cost. If
    // less cost the predecessor is updated and the sort cost is decremented
    // by the difference in real cost (A* heuristic doesn't change)
    if (edgestatus.set() == EdgeSet::kTemporary) {
      CheckIfLowerCostPathForward(edgestatus.status.index, pred_idx, newcost, tc);
      continue;
    }
...
void BidirectionalAStar::SetOrigin(GraphReader& graphreader,
  ...
  for (const auto& edge : origin.edges) {
    ...
    Cost cost = costing_->EdgeCost(directededge,
...
void BidirectionalAStar::SetDestination(GraphReader& graphreader,
  ...
  for (const auto& edge : dest.edges) {
    ...
    // edge.
    Cost cost = costing_->EdgeCost(opp_dir_edge,
